...
matrix_test <- read_tsv(paste0(sample_ID, ".mat_values.tab"), skip = 3, col_names = as.character(c(1:430)))
...
write.table(matrix_test_zero_gather, paste0(sample_ID, "_mat_values_summarised.tab"), sep = "\t", quote = F, row.names = F)
...
ggsave(filename =paste0(sample_ID, "_mat_metaplot_2.pdf"), plot =  g, width = 3, height = 3)
