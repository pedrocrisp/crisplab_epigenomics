...
usage="USAGE:
bash 04-filter-bwa-meth_qsub.sh <sample_list.txt> <paired_end> <walltime> <memory>
for example:
bash \
crisplab_methylation/methylome/04-filter-WGBS-regular_qsub.sh \
single_sample.txt \
yes \
24:00:00 \
40
...
qsub -J $qsub_t \
-l walltime=${walltime},nodes=1:ppn=24,mem=${mem}gb \
